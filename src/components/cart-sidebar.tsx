...
export function CartSidebar() {
  const { cart, clearCart, cartTotal, isCartOpen } = useCart();
  ...
  return (
    <div
      className="bg-white border-start d-flex flex-column"
    ...
    >
      ...
      {isCartOpen ? (
        cart.length > 0 ? (
          ...
          <div className="flex-grow-1 d-flex flex-column align-items-center justify-content-center p-4 text-center">
            <div className="bg-light rounded-circle p-4 mb-3">
              <FontAwesomeIcon icon={faShoppingCart} className="fa-2x text-secondary" />
            </div>
        ...
        <div className="flex-grow-1 d-flex align-items-center justify-content-center">
          <p className="text-secondary">Carrito cerrado</p>
        </div>
      )}
